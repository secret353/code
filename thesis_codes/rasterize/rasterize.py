...
rasterize_cuda = load(
    "rasterize_cuda",
    [
        "/root/autodl-fs/thesis_codes/rasterize/rasterize_cuda.cpp",
        "/root/autodl-fs/thesis_codes/rasterize/rasterize_cuda_kernel.cu",
    ],
    verbose=True,
)
# rasterize_cuda = load(
...
class RasterizeFunction(Function):
    ...
    def forward(ctx, vertices, faces, shape):
        ...
        # shape = torch.tensor([H, W, D]).int().cuda()
        volume = []
    ...
    def backward(ctx, grad_output):
        ...
        for output_, grad_volume_, vertices_, faces_ in zip(
            volume, grad_volume, vertices, faces
        ):
            # v = (shape[None].float() - 1) * (vertices_.clone() + 1)/2
            v = vertices_.clone()
            v = torch.round(v).float()
            ...
            grad_vertices_ = rasterize_cuda.backward(
                output_, grad_volume_, v, f, shape
            )[0].float()[None]
